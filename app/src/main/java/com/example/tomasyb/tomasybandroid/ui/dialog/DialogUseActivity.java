package com.example.tomasyb.tomasybandroid.ui.dialog;
...
import com.example.tomasyb.baselib.widget.dialog.CustomDialog;
import com.example.tomasyb.baselib.widget.dialog.EditDialog;
 ...
 * dialog使用
 */
public class DialogUseActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void showEnsureDialogOne() {
        ensureDialog = new EnsureDialog(this).builder()
                ...
                .setTitle("这里是一个标题", getResources().getColor(R.color.b_main_black))//可以不设置标题颜色，默认系统颜色
                .setCancelable(false)
                ...
                .setPositiveButton("确认", getResources().getColor(R.color.b_main_red), new View.OnClickListener() {//可以选择设置颜色和不设置颜色两个方法
                    @Override
                    public void onClick(View view) {
    ...
    private void showEnsureDialogThree() {
        ensureDialog = new EnsureDialog(this).builder()
                .setGravity(Gravity.CENTER)//默认居中，可以不设置
                .setTitle("这里是一个标题", getResources().getColor(R.color.b_main_black))//可以不设置标题颜色，默认系统颜色
                .setIncon(R.drawable.ic_qq)//不设置图标，默认没有图标
                .setCancelable(false)
                .setNegativeButton("取消", new View.OnClickListener() {//可以选择设置颜色和不设置颜色两个方法
                    @Override
                    public void onClick(View view) {
    ...
    private void showEnsureDialogFour() {
        ensureDialog = new EnsureDialog(this).builder()
                .setGravity(Gravity.CENTER)//默认居中，可以不设置
                .setTitle("这里是一个标题", R.color.b_main_black)//可以不设置标题颜色，默认系统颜色
                .setCancelable(false)
    ...
    private void showBottomPopupWindow() {
        new BottomPopupWindow(this).builder()
                ...
                .addSheetItem("相机",  ContextCompat.getColor(this, R.color.main), new BottomPopupWindow.OnSheetItemClickListener() {
                    @Override
                    public void onClick(int which) {
                        //需要对相机进行运行时权限的申请
                    }
                })
                .addSheetItem("相册", ContextCompat.getColor(this, R.color.b_main_black), new BottomPopupWindow.OnSheetItemClickListener() {
                    @Override
                    public void onClick(int which) {
                        //调用手机相册的方法,该方法在下面有具体实现
                    }
                })
                .addSheetItem("时光机",  ContextCompat.getColor(this, R.color.b_main_black), new BottomPopupWindow.OnSheetItemClickListener() {
                    @Override
                    public void onClick(int which) {
    ...
    private void showBottomPopupWindow1() {
        new BottomPopupWindow(this).builder()
                .setCancelable(false).setCanceled(true)
                .addSheetItem("相机",  ContextCompat.getColor(this, R.color.b_main_black), new BottomPopupWindow.OnSheetItemClickListener() {
                    @Override
                    public void onClick(int which) {
                        //需要对相机进行运行时权限的申请
                    }
    ...
    private void showSDCustomEditDialog() {
        ...
        btnSure.setTextColor(ContextCompat.getColor(this, R.color.b_text_gray));
